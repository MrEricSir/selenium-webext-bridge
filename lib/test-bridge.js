...
class TestBridge {
  ...
  async waitForUrl(pattern, timeout = 10000) {
    ...
    return await this.driver.executeScript((p, t) => {
      return window.TestBridge.waitForUrl(p, t);
    }, pattern, timeout);
  }
   ...
   * @param {*} timeout Maximum time to wait in milliseconds; defaults to 5000
   * @returns The result of the condition function
   */
  async waitFor(conditionFn, timeout = 5000) {
    await this.ensureReady();
    return await this.driver.executeScript((timeoutMs) => {
      return window.TestBridge.waitFor(
        conditionFn,
        timeoutMs
      );
    }, timeout);

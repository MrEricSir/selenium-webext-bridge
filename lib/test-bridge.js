...
class TestBridge {
  ...
  async init() {
    try {
      ...
      // Give page and extension time to load.
      // TODO: Is this needed?
      await sleep(2500);
  ...
  /**
   * Waits for a condition function to return a truthy value.
   * Polls on the Node.js side, so conditionFn can call bridge methods.
   * For browser-side waiting, use driver.executeScript with window.TestBridge.waitFor directly.
   * @param {Function} conditionFn Async function that returns a truthy value when done
   ...
   */
  async waitForCondition(conditionFn, timeout = 5000, interval = 250) {
    const startTime = Date.now();
